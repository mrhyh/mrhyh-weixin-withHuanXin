...
@interface TLChatBoxViewController () <TLChatBoxDelegate, TLChatBoxFaceViewDelegate, TLChatBoxMoreViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, EMLocationViewDelegate, EMChooseViewDelegate>
...
- (void) chatBoxMoreView:(TLChatBoxMoreView *)chatBoxMoreView didSelectItem:(TLChatBoxItem)itemType
{
    if (itemType == TLChatBoxItemAlbum) {            // 相册
    ...
    } else if(itemType == TLChatBoxItemCallingCard){
        ...
        EMMessage *message = [[EMMessage alloc] initWithReceiver:@"two" bodies:@[body]];
...
//        [alert show];
        ...
        EaseLocationViewController *locationVC = [[EaseLocationViewController alloc] init];
...
#warning 有时间再优化，传image对象耗资源，传个地址，发送时再根据地址读取图片更好
    message.image = image; //
    if (_delegate && [_delegate respondsToSelector:@selector(chatBoxViewController:sendMessage:)]) {
        [_delegate chatBoxViewController:self sendMessage:message];
    }
...
- (TLChatBoxMoreView *) chatBoxMoreView
{
    if (_chatBoxMoreView == nil) {
        ...
        TLChatBoxMoreItem *videoItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"小视频"imageName:@"sharemore_sight"];
        TLChatBoxMoreItem *videoCallItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"视频聊天"imageName:@"sharemore_videovoip"];
        TLChatBoxMoreItem *giftItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"红包"imageName:@"sharemore_wallet"];
        TLChatBoxMoreItem *transferItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"转账"imageName:@"sharemorePay"];
        TLChatBoxMoreItem *positionItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"位置"imageName:@"sharemore_location"];
        TLChatBoxMoreItem *favoriteItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"收藏"imageName:@"sharemore_myfav"];
        TLChatBoxMoreItem *businessCardItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"名片"imageName:@"sharemore_friendcard" ];
        ...
        TLChatBoxMoreItem *cardsItem = [TLChatBoxMoreItem createChatBoxMoreItemWithTitle:@"卡券"imageName:@"sharemore_wallet"];
        ...
        [_chatBoxMoreView setItems:[[NSMutableArray alloc] initWithObjects:photosItem, takePictureItem, businessCardItem, positionItem, nil]];
    }
...
-(void)sendLocationLatitude:(double)latitude
    ...
    TLMessage *message = [[TLMessage alloc] init];
    message.messageType = TLMessageTypeLocation;
    message.ownerTyper = TLMessageOwnerTypeSelf;
    message.date = [NSDate date];
   
    CLLocationCoordinate2D clLocationCoordinate2D;
    clLocationCoordinate2D.latitude = latitude;
    clLocationCoordinate2D.longitude = longitude;
    
    message.coordinate  = clLocationCoordinate2D;
...
- (BOOL)viewController:(EMChooseViewController *)viewController didFinishSelectedSources:(NSArray *)selectedSources{
    ...
    for(EMBuddy *buddy in selectedSources){
        ...
        EMChatText *txt = [[EMChatText alloc] initWithText:buddy.username];
        EMTextMessageBody *body = [[EMTextMessageBody alloc] initWithChatObject:txt];
        EMMessage *message = [[EMMessage alloc] initWithReceiver:@"two" bodies:@[body]];
        message.messageType = eMessageTypeChat; // 设置为单聊消息
        //message.messageType = eConversationTypeGroupChat;// 设置为群聊消息
