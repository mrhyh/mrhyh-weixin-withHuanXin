...
- (void) setMessage:(TLMessage *)message
{
    ...
    switch (message.ownerTyper) {
        ...
        {
            [self.avatarImageView setHidden:NO];
            UIImage *avatarImage = [UIImage imageNamed:message.from.avatarURL];
            if(avatarImage != nil){
                [self.avatarImageView setImage:avatarImage];
            }else {
                [self.avatarImageView setImage:[UIImage imageNamed:@"ToolViewEmotion"]];
            }
        ...
        case TLMessageOwnerTypeUnknown:
            NSLog(@"未知的消息拥有者...");
            break;
