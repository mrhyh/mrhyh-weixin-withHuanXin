...
@interface GroupListViewController ()<UISearchBarDelegate, UISearchDisplayDelegate, SRRefreshDelegate>

//IChatManagerDelegate

@property (strong, nonatomic) NSMutableArray *dataSource; //群组列表数据
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        user.username = group.groupId;  //群聊才用groupId
        user.groupSubject = group.groupSubject;
        user.eMmessageType = eConversationTypeGroupChat;
        
        TLChatViewController *tlChatVC = [[TLChatViewController alloc] init];
        tlChatVC.user = user;
        [self setHidesBottomBarWhenPushed:YES];
        [self.navigationController pushViewController:tlChatVC animated:YES];
